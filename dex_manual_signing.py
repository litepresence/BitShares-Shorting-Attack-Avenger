# =======================================================================
VERSION = "Bitshares manualSIGNING 0.00000007"
# =======================================================================
...
if version_info[0] < 3:
    raise Exception("% is DED, long live Python 3.7+" % version_info[0])
...
def sample_orders():
    ...
    order4 = {
        edicts: [{"op": "call", "debt": 0, "collateral": 0,}],
        "header": {
            "asset_id": "1.3.0",
            "currency_id": "1.3.861",
            "asset_precision": 5,
...
def global_constants():
    ...
    OP_IDS = {"Limit_order_create": 1, "Limit_order_cancel": 2, "Call_order_update": 3}
...
def rpc_fees():
    ...
    query = [
        ...
        [
            [
                ...
                ["3", {"from": str(account_id)}],  # FIXME confirm op is 3
            ],
            "1.3.0",
        ],
    ]
    ...
    cancel = ret[1]["amount"]
    call = ret[2]["amount"]
    return {"create": create, "cancel": cancel, "call": call}
...
def serialize_transaction(tx):
    ...
    rpc_tx_hex = rpc_get_transaction_hex_without_sig(json_loads(json_dumps(tx)))
